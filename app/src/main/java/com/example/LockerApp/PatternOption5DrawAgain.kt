...
import com.example.LockerApp.databinding.ActivityPatternOption5DrawAgainBinding
import com.example.patternlock.PatternLockView

class PatternOption5DrawAgain : AppCompatActivity() {
    private lateinit var binding: ActivityPatternOption5DrawAgainBinding
    private lateinit var patternLockView: PatternLockView

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityPatternOption5DrawAgainBinding.inflate(layoutInflater)
        setContentView(binding.root)

        binding.backArrow.setOnClickListener{
            startActivity(Intent(this@PatternOption5DrawAgain,PatternOption5Draw::class.java))
        }
        ...
        binding.patternLockViewDisplay.setOnPatternListener(object : PatternLockView.OnPatternListener {
            ...
            override fun onComplete(ids: ArrayList<Int>): Boolean {
                ...
                return if (savedPattern == modifiedEnteredPattern) {
                    Toast.makeText(
                        this@PatternOption5DrawAgain,
                        "Pattern Correct!",
                        Toast.LENGTH_SHORT
                    ).show()
                    // Proceed to the next activity if the pattern matches
                    startActivity(Intent(this@PatternOption5DrawAgain, AppLockFreeActivity::class.java))
                    true
                } else {
                    Toast.makeText(
                        this@PatternOption5DrawAgain,
