...
class PatternOption4Draw : AppCompatActivity() {
    private lateinit var patternLockView: PatternLockView
    private lateinit var binding: ActivityPatternOption4DrawBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityPatternOption4DrawBinding.inflate(layoutInflater)
        setContentView(binding.root)
        binding.backArrow.setOnClickListener{
            startActivity(Intent(this@PatternOption4Draw, PatterndetailsActivity4::class.java))
        }
        binding.textView8.setOnClickListener{
            startActivity(Intent(this@PatternOption4Draw, PinStyle::class.java))
        }
        binding.patternLockViewDisplay.setOnPatternListener(object : PatternLockView.OnPatternListener {
            ...
            override fun onComplete(ids: ArrayList<Int>): Boolean {
                if (ids.size > 2) {
                    savePattern(ids)
                    startActivity(Intent(this@PatternOption4Draw, PatternOption4DrawAgain::class.java))
                    return true
                }
                return false
            }
