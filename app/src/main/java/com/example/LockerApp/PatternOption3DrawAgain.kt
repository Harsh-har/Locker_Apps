...
import com.example.LockerApp.databinding.ActivityPatternOption3DrawAgainBinding
...
class PatternOption3DrawAgain : AppCompatActivity() {
    private lateinit var binding: ActivityPatternOption3DrawAgainBinding
    private lateinit var patternLockView: PatternLockView

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityPatternOption3DrawAgainBinding.inflate(layoutInflater)
        ...
        binding.patternLockViewDisplay.setOnPatternListener(object : PatternLockView.OnPatternListener {
            ...
            override fun onComplete(ids: ArrayList<Int>): Boolean {
                ...
                return if (savedPattern == modifiedEnteredPattern) {
                    Toast.makeText(
                        this@PatternOption3DrawAgain,
                        "Pattern Correct!",
                        Toast.LENGTH_SHORT
                    ).show()
                    // Proceed to the next activity if the pattern matches
                    startActivity(Intent(this@PatternOption3DrawAgain, AppLockFreeActivity::class.java))
                    true
                } else {
                    Toast.makeText(
                        this@PatternOption3DrawAgain,
